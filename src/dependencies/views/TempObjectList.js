 ...
 * 
 */
class TempObjectList extends Component {
    constructor(props) {
        super(props)
    
        this.state = {
             theadList: [], actionColumn: this.props.actionColumn, ul: this.props.ul, customButtonLabel: this.props.customButtonLabel, linkLabel: this.props.linkLabel, dataList: this.props.data, total: this.props.total,
    ...
    componentDidUpdate(props, state) {
        ...
        if (state.offset !== this.state.offset) {
    ...
    firstPage = () => {
        ...
        if (this.state.currentPage > 1) {
            this.props.onChange(this.state.dataPerPage, offset);
        }
    }
    ...
    handleColDoubleClick = (ref) => {
        if (this.props.inlineEdit !== undefined) {
                                    ...
                                    <tbody className="zero-container-main">
                                        <ZeroContainer data={this.state.dataList} placeholder={this.zeroContainerPlaceholder}>
                                        {
                                            this.state.dataList.map((item, i) => (
                                                <tr data-position={i} className={(JArray.find.getBoolean(this.state.rowsSelected, item.id))? "row-selected" : ""} key={i} onClick={this.handleRowClick.bind(null, "ref"+i,item)} ref={this["ref"+i]}>
                                                    {
                                                        this.getKeys(item).map((key, j) => (
                                                            <React.Fragment key={j}> 
                                                            {
                                                                ...
                                                                :
                                                                    <td ref={this["ref"+i+j]} onDoubleClick={this.handleColDoubleClick.bind(null, "ref"+i+j)}> 
                                                                    {
                                    ...
                                    this.props.showNavigation !== false ?
                                        <div className="two">
                                            <div className="content-left">
                                                <button title="First page" className={(this.state.offset === 0)?"active":""} onClick={this.firstPage} disabled={(this.state.offset === 0)? true: false}> <FaAngleDoubleLeft /> </button>
                                                <button title="Prev page" onClick={this.prevPage} disabled={(this.state.currentPage - 1 === 0)? true: false}><GoChevronLeft /></button>
                                                {
                                                ...
                                                <button title="Last Page" className={(this.state.currentPage >= Math.ceil(this.props.total / this.state.dataPerPage))?"active":""} onClick={this.lastPage.bind(null)} disabled={(this.state.currentPage >= Math.ceil(this.props.total / this.state.dataPerPage))? true : false}><FaAngleDoubleRight /></button>
                                                {
                                                    ...
                                                    <>
                                                        <span> Showing {this.state.offset + 1} to {this.state.offset + this.state.dataPerPage} of {this.state.total} records</span>
