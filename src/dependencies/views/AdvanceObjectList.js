 ...
 * 
 */
class AdvanceObjectList extends Component {
    constructor(props) {
        super(props)
    ...
    nextPage = () => {
        const newPage = this.state.currentPage + 1;
        const offset = (newPage * this.state.dataPerPage) - this.state.dataPerPage;
        this.setState({currentPage: newPage}, ()=>this.props.onChange(offset, 5));
    };
  ...
  render() {
    ...
    // Check if an array or object exist as a child of the data
        ...
        const showingTo = this.state.offset + this.state.dataPerPage;
        console.log("RENDER" , this.state.offset)
                            ...
                            <tbody>
                                <ZeroContainer data={this.state.dataList} placeholder={this.zeroContainerPlaceholder}>
                                {
                                    this.state.dataList.map((item, i) => (
                                        <tr data-position={i} className={(JArray.find.getBoolean(this.state.rowsSelected, item.id) && this.props.rowSelection === true)? "row-selected" : ""} key={i} onClick={this.handleRowClick.bind(null, "ref"+i,item)} ref={this["ref"+i]}>
                                            {
                                ...
                                <div className="two">
                                    <div className="content-left">
                                        ...
                                        <button title="Last Page" className={(this.state.currentPage >= Math.ceil(this.props.total / this.state.dataPerPage))?"active":""} onClick={this.lastPage.bind(null)} disabled={(this.state.currentPage >= Math.ceil(this.props.total / this.state.dataPerPage))? true : false}><FaAngleDoubleRight /></button>
                                        {
                                            ...
                                            <>
                                                <span> Showing {this.state.offset + 1} to {( this.state.total < showingTo) ? this.state.total : showingTo} of {this.state.total} record{this.state.total > 1?"s":null}</span>
...
export default AdvanceObjectList;
