...
class TableToExcel extends Component {
  ...
  handleDownload() {
    ...
    try {
      ...
      try {
        table = document.getElementsByTagName("table")[0].outerHTML;        
      } catch (error) {
    ...
    element.href =
      uri +
      TableToExcel.base64(
        TableToExcel.format(template, context),
      );
  ...
  render() {
    return (
      ...
      >
        {this.props.buttonText} <MdPrint/>
      </button>
    );
  }
}

TableToExcel.propTypes = propTypes;
TableToExcel.defaultProps = defaultProps;
